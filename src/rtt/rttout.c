 ...
 */
static void tnd_var(struct sym_entry *sym, char *strct_ptr,
   char *access, int indent)
   {
   /*
 ...
 */
static void cnv_fnc(struct token *t, int typcd,
   struct node *src, struct node *dflt, struct node *dest, int indent)
   {
   int dflt_to_ptr;
   int loc;
   int is_cstr;
 ...
 */
static void ret_1_arg(struct token *t, struct node *args,
   int typcd, char *vwrd_asgn, char *arg_rep, int indent)
   {
   if (args == NULL || args->nd_id == CommaNd)
...
static int typ_case(struct node *var, struct node *slct_lst, struct node *dflt,
   int (*walk)(struct node *n,int xindent,int brace), int maybe_var, int indent)
   {
   struct node *lst;
   struct node *select;
   struct node *slctor;
 ...
 */
static int len_sel(struct node *sel,
   struct parminfo *strt_prms, struct parminfo *end_prms, int indent)
   {
   int fall_thru;
...
static void comp_def(struct node *n)
   ...
   if (op_type != Keyword) {
       ...
       */
      fprintf(out_file, "int %c%c%c_%s(",
         uc_letter, prfx1, prfx2, cur_impl->name);
      fprintf(out_file,
         "int r_nargs, dptr r_args, dptr r_rslt, continuation r_s_cont)");
      fname = cname;
      line = 8;
      ForceNl();
      prt_str("{", IndentInc);
