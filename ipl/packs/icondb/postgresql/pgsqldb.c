/*-----------------8/29/2007 11:43AM-----------------
 * loadable C function pgsqldb for icon access to
 * a postgreSQL database from linux, by Carl Sturtivant.
 * (This also built and ran on solaris.)
 *
 * This should be Garbage Collection safe.
 *
 * Requires a postgreSQL installation to build.
 * I used the following from bash:

CFG=pg_config
bash -c "gcc -shared -o pgsqldb.so -fPIC -I`$CFG --includedir` pgsqldb.c -L`$CFG --libdir` -lpq"

 * on solaris I also needed -lssl -lcrypt
 *
 * For details about calling pgsqldb, see below.
 * --------------------------------------------------*/
...
/* make icon list of postgreSQL error information */
static descriptor error_info(int pgsqlNumber, const char *pgsqlError) {
    descriptor number;
    struct {
        gcu_tended other;
        descriptor text, ls;
    ...
    tended.ls = newlist(2);
    Mkinteger(pgsqlNumber, &number);
    Mkstring((char *)pgsqlError, &tended.text);
...
/* make postgreSQL row retrieved from query results into icon list */
static descriptor convertrow(PGresult *result, int rownum, int numfields) {
    int i;
    struct {
        gcu_tended other;
        descriptor x, ls;
    ...
    for( i = 1; i <= numfields; ++i ) {
        if( !PQgetisnull(result, rownum, i-1) )
            Mkstring( PQgetvalue(result, rownum, i-1), &tended.x );
...
/*--------------------------------------------------
 * Called with a list, pgsqldb attempts to connect.
 ...
 *
 * Called with a string, pgsqldb attempts to
 * execute that string as a postgreSQL query.
 *
 * Called with no parameters, pgsqldb closes
 ...
 * result is an error.
 * --------------------------------------------------*/
int pgsqldb(int argc, descriptor argv[]) {
    static PGconn *dbh; /* connection sticks around between calls */
    static int connected = 0;

    PGresult *result;
    char *querystring, *hoststring, *portstring,
        *databasestring, *userstring, *passwordstring, *num_rows;
    int i, len, rowsize, error_code, num_records;
    struct {
        gcu_tended other;
    ...
    if( argc == 0 ) {  /* close connection */
        if( connected ) PQfinish(dbh);
        connected = 0;
        gcu_aware_pop();
        Fail;
    } /* end close connection */

    if( argc >= 1 && IconType(argv[1]) == 'L' ) { /* connect to postgreSQL */
        if( connected )
            ReturnDescriptor( error_info(-1, "pgsqldb: already connected") );

        tended.ls = argv[1];
        hoststring = "localhost"; /* host defaults to localhost */
        portstring = NULL; /* Gives the postgreSQL default */

        switch( ListLen(tended.ls) ) {
            default:
                ReturnDescriptor( error_info(-1, "pgsqldb: list of dbname, user, pwd, [host, [port]] expected") );
            case 5 :
                tended.port = subscript(tended.ls, 5);
                if( !cnv_int(&tended.port,&tended.port) ) ReturnError(tended.port,101);
                cnv_str(&tended.port,&tended.port);
                portstring = StringVal(tended.port);
            case 4 :
        ...
        dbh = PQsetdbLogin(hoststring, portstring, NULL, NULL,
        ...
        if( PQstatus(dbh) == CONNECTION_OK ) {
            ...
            PQfinish(dbh);
            ReturnDescriptor( error_info(-1, "pgsqldb: attempt to connect failed")  );
        }
    } /* end connect to postgreSQL */

    if( argc >= 1 && IconType(argv[1]) == 's' )  { /* execute a query */
        if( !connected )
            ReturnDescriptor( error_info(-1, "pgsqldb: not connected") );
        querystring = StringVal(argv[1]);

        result = PQexec(dbh, querystring);

        if( !result ) /* fatal error */
            ReturnDescriptor( error_info(PGRES_FATAL_ERROR, PQerrorMessage(dbh))  );

        error_code = PQresultStatus(result);

        switch( error_code ) {
            default: /* some sort of error */
                PQclear(result);
                ReturnDescriptor( error_info(error_code, PQerrorMessage(dbh))  );

            case PGRES_COMMAND_OK: /* not a SELECT query */
                num_rows = PQcmdTuples(result);
                PQclear(result);
                if( strlen(num_rows)==0 ) num_rows = "0";
                gcu_aware_pop();
                RetInteger( atoi(num_rows) );

            case PGRES_TUPLES_OK: /* SELECT query */
                num_records = PQntuples(result);
                tended.answer = newlist( num_records );
                rowsize = PQnfields(result);
                for( i=0; i<num_records; ++i )
                    assign( tended.answer, i+1, convertrow(result, i, rowsize) );
                PQclear(result);
                ReturnDescriptor(tended.answer);
        }
    } /* end execute a query */

    /* wrong argument type to pgsqldb */
